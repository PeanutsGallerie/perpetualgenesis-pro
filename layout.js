...
function renderTimeline() {
    // Timeline should follow the SAME plan set as My Garden / Perpetual
    // so perpetual-applied entries show up here and plan switching stays in sync.
    try {
        if (typeof renderPlanTabs === 'function') {
            // If your plan-tab system supports (scope, containerId, onChange) use it when available.
            const el = document.getElementById('timelinePlanTabs');
            if (el && renderPlanTabs.length >= 3) renderPlanTabs('mygarden', 'timelinePlanTabs', renderTimeline);
            else renderPlanTabs('mygarden');
        }
    } catch (e) {}
